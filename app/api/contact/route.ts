...
export async function POST(req: NextRequest) {
    ...
    const { error: resendError } = await resend.emails.send({
        ...
        <div style="font-family:monospace;max-width:600px;margin:auto;padding:24px;background:#0a0a0f;color:#f0f0ff;border:1px solid #1a1a3e;border-radius:8px">
            ...
            <tr><td style="color:#4a4a7a;padding:8px 0;width:80px">NOMBRE</td><td style="color:#f0f0ff">${name}</td></tr>
            <tr><td style="color:#4a4a7a;padding:8px 0">EMAIL</td><td><a href="mailto:${email}" style="color:#00ff41">${email}</a></td></tr>
            </table>
            <hr style="border-color:#1a1a3e;margin:16px 0"/>
            <p style="color:#4a4a7a;margin:0 0 8px">MENSAJE:</p>
